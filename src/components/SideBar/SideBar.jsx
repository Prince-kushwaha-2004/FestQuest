...
const TitleSection = ({ open }) => {
  return (
    <div className="mb-10 mt-4 pb-3  ">
      <div
        className="flex cursor-pointer bg-white items-center justify-between shadow-md rounded-2xl transition-all hover:scale-103 hover:shadow-lg"

      >
        <div className={"flex py-2 items-center gap-2" + open ? " px-8" : "px-4"}>
          <motion.div
            layout
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {open ?
              <img src={logo} alt="" /> :
              <img src={smallLogo} className="h-9 w-5 
              " alt="" />
            }
          </motion.div>
